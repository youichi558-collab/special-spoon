...
class CatalogDB:
    ...
    def build(self, verbose=False):
        ...
        #
        # 【2026-09-21修正】以前はここで**本体(拡張子なし)も消してから**
        # os.replace していた。os.replace は既存ファイルを不可分に上書きするので
        # 消す必要が無く、消すせいで「本体が存在しない数ミリ秒」をわざわざ
        # 作っていた。その隙間に検索が入ると _connect() の mode=ro が
        # 「ファイルが無い」で失敗する。
        #
        # server.py をシングルスレッドで動かしていた間は、検索要求が再構築の
        # 後ろに並ぶだけで踏めなかった。2026-09-21にThreadingHTTPServerへ
        # 変えて同時に動くようになったため、踏めるようになっていた。
        #
        # 鍵をかけて順番待ちにする案もあったが、それだと**再構築が終わるまで
        # 検索が数秒固まる**という別の不便を持ち込む。隙間自体を無くせば
        # 待ち時間ゼロで済む。差し替わる瞬間に検索していた場合は、古い内容か
        # 新しい内容のどちらかが返る —— どちらも正しい内容。
        #
        # -wal/-shm を**先に**消すのは、あれが古いDBの道連れだから。
        # 本体を差し替えた後に残っていると、新しい本体と古いジャーナルが
        # 組み合わさる一瞬ができる。
        for ext in ('-wal', '-shm'):
            p = self.db_path + ext
            if os.path.exists(p):
