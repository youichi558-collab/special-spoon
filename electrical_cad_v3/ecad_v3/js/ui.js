...
// （推測で入れて外すより、聞いた方がよい）。
//
// 【2026-09-21】種別を追加した(盛田さん「種別を増やしてくれ、仮設定を追加、
// 自動選択にのせるように」)。登録シンボル31個中17個が種別未設定だったのは、
// **サーマル・限時接点・接点1〜4に当たる選択肢が無かった**ため。
// 拾う名前は部品DB(catalog_pending/*.csv)の実データを数えて決めた:
//   サーマル接点 38 / 限時接点・限時接点1〜4 44 / 接点1〜4 38 / 制御入力 33
//
// `tentative`(仮設定)はここに**入れない**。後で決めるための目印なので、
// 自動選択には乗せない(matchGroupsByRole が [] を返し、従来どおり端子点数の
// 判定へ落ちる)。
const TERM_GROUP_PATTERNS = {
  ...
  contact_thermal: [/サーマル/],
  contact_timer:   [/限時/],
  // 「接点1」「接点2」…だけを拾う。**行頭から末尾まで**で縛らないと
  // 「主接点」「サーマル接点」「限時接点1」まで巻き込む。
  contact_num:     [/^\s*接点\s*\d*\s*$/],
  ctrl_in:         [/制御入力/, /リセット入力/],
};
