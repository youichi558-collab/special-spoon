...
function getAllSnapPoints(wx, wy) {
    ...
    if (state.snapEnd && !['text','rect','circle','fline','dim','leader'].includes(el.type)) {
      ...
      if (cS && cS.terminals?.length) {
        cS.terminals.forEach(t => {
          ...
          if (dist < bestD) { bestD = dist; best = { x: el.x+rx, y: el.y+ry, type:'endpoint' }; }
        });
      } else {
        const hw = (d.w||0)/2;
        [+hw, -hw].forEach(dx => {
  ...
  state.wires.forEach(w => {
    ...
    pts.forEach(p => {
      ...
      if (state.snapEnd && d < bestD) { bestD = d; best = { x:p.x, y:p.y, type:'endpoint' }; }
    });
    if (state.snapMid) {
      for (let i = 0; i < pts.length-1; i++) {
        const mx = (pts[i].x + pts[i+1].x)/2, my = (pts[i].y + pts[i+1].y)/2;
        const d = Math.hypot(wx - mx, wy - my);
        if (d < bestD) { bestD = d; best = { x:mx, y:my, type:'midpoint' }; }
      }
    }
  });
...
function snapWirePoint(wx, wy, prevX, prevY) {
  ...
  if (sp) {
    if (state.ortho && prevX != null) { const o = applyOrtho(prevX, prevY, sp.x, sp.y); return { x:o.x, y:o.y, snapType:sp.type }; }
    return { x:sp.x, y:sp.y, snapType:sp.type };
  }
